...
public class PonyIfTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyIfTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.ELSEIF_ )
                {
                    block = new PonyElseIfTermBlock( child, getAlignment(), spacingBuilder );
                }
