...
public class PonyTypeParamBlock extends AbstractPonyBlock
{
    protected PonyTypeParamBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.TYPEARG )
                {
                    block = new PonyTypeBlock( child, getAlignment(), spacingBuilder );
                }
                else
