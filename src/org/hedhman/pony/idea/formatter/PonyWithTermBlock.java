...
public class PonyWithTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyWithTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.WITHELEM )
                {
                    block = new PonyWithElemBlock( child, getAlignment(), spacingBuilder );
                }
