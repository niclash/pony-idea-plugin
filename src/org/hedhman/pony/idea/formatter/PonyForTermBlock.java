...
public class PonyForTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyForTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.IDSEQ )
                {
                    block = new PonyIdSeqBlock( child, getAlignment(), spacingBuilder );
                }
