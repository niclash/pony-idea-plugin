...
public class PonyNoSemiBlock extends AbstractPonyBlock
    implements Block
{
    public PonyNoSemiBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            ...
            {
                Block block;
                if( child.getElementType() == PonyTypes.NEXTEXPRSEQ )
                {
                    block = new PonyExprSeqBlock( child, getAlignment(), spacingBuilder );
                }
                else if( child.getElementType() == PonyTypes.EXPRSEQ )
                {
                    block = new PonyExprSeqBlock( child, getAlignment(), spacingBuilder );
                }
