...
public class PonyIfDefTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyIfDefTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.ELSEIFDEF )
                {
                    block = new PonyElseIfDefBlock( child, getAlignment(), spacingBuilder );
                }
                else
                {
