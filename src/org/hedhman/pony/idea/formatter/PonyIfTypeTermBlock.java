...
public class PonyIfTypeTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyIfTypeTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.IFTYPE_ )
                {
                    block = new PonyIfTypeBlock( child, getAlignment(), spacingBuilder );
                }
                else if( child.getElementType() == PonyTypes.ELSEIFTYPE )
                {
                    block = new PonyElseIfTypeBlock( child, getAlignment(), spacingBuilder );
                }
                else
                {
