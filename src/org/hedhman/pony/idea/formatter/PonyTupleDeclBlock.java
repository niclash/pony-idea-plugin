...
public class PonyTupleDeclBlock extends AbstractPonyBlock
    implements Block
{
    public PonyTupleDeclBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            ...
            {
                Block block;
                if( child.getElementType() == PonyTypes.RAWSEQ )
                {
                    block = new PonyRawSeqBlock( child, getAlignment(), spacingBuilder );
                }
                else if( child.getElementType() == PonyTypes.TUPLE )
                {
                    block = new PonyTupleBlock( child, getAlignment(), spacingBuilder );
                }
                else
                {
