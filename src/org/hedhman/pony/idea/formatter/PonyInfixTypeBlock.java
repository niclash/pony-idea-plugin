...
public class PonyInfixTypeBlock extends AbstractPonyBlock
    implements Block
{
    public PonyInfixTypeBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.UNIONTYPE )
                {
                    block = new PonyUnionTypeBlock( child, getAlignment(), spacingBuilder );
                }
                else if( child.getElementType() == PonyTypes.ISECTTYPE )
                {
                    block = new PonyIsectTypeBlock( child, getAlignment(), spacingBuilder );
                }
                else
                {
