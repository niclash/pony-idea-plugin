...
public class PonyMatchTermBlock extends AbstractPonyBlock
    implements Block
{
    public PonyMatchTermBlock( @NotNull ASTNode node, @Nullable Alignment alignment, SpacingBuilder spacingBuilder )
    {
    ...
    protected List<Block> buildChildren()
    {
        ...
        while( child != null )
        {
            if( handleCommentAndWhitespaceBlock( blocks, child, getAlignment(), spacingBuilder ) )
            {
                ...
                else if( child.getElementType() == PonyTypes.CASEEXPR )
                {
                    block = new PonyCaseExprBlock( child, getAlignment(), spacingBuilder );
                }
                else
                {
